        ...
        while en_cours:
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    ...
                    colonne = position[0] // 100
                    rangee = position[1] // 100
                    print(colonne)
                    print(rangee)
                    try:
                        tableau[rangee][colonne] = 1
                    except IndexError:
                        print("wrong")
            ...
            self.draw_grid(tableau)
            # print(f"je suis x {x}")
            # print(f"je suis y {y}")

            pygame.display.update()

    def draw_grid(self, test):
        taille_bloc = 45
        for rangee in range(2, len(tableau)):
            for colonne in range(1, len(tableau[rangee]) - 1):
                if tableau[rangee][colonne] == 1:
                    print("BLANC")
                rect = pygame.Rect(rangee * taille_bloc, colonne * taille_bloc, taille_bloc, taille_bloc)
